...
Based on mag with independent decay thresholds:

| Parameter | Formula | Notes |
|-----------|---------|-------|
| Alpha | `10^(-0.05 × max(0, mag-1))` | Pogson's law, full at mag≤1 |
| RGB | Same as Alpha | Color & alpha decay together |
| Size | `max(10^(-0.05 × max(0, mag-2)), 0.3) × starCoreSize` | Minimum visible size for dim stars |

Glow quads only for mag < 2.0: `< 0.5 → high / < 1.5 → medium / < 2.0 → low`.
...
| `mag` | Apparent magnitude (Gaia G-band or BSC5 V-band). Determines glow tier and brightness decay |
...
|---------|------|-------------|
| `/readstar skybox mag <0~10>` | `mag` — magnitude threshold | Rebuild star buffer filtered by mag. Only stars with `mag ≤ threshold` are rendered. Default `6.0` (naked-eye limit). Client-side, instant effect. |
